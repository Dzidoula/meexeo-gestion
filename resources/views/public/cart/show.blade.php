<x-layouts.public title="Mon panier — SONOR LOCATION">
    <div class="mx-auto max-w-4xl px-6 py-10">
        <h1 style="font-size:26px;font-weight:800">Mon panier</h1>

        @if (session('cart_status'))
            <p class="mt-3 text-sm" style="color:var(--color-sonor-ink-soft)">{{ session('cart_status') }}</p>
        @endif
                ...
                @foreach ($vehicles as $vehicle)
                    <div class="flex items-center justify-between gap-4 p-4" style="border-radius:var(--radius-sonor);border:1px solid var(--color-sonor-border)">
                        ...
                        <form method="POST" action="{{ route('cart.remove', $vehicle) }}">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="min-h-[44px] text-sm font-semibold" style="border-radius:var(--radius-sonor-sm);border:1px solid var(--color-sonor-border);background:transparent;padding:0 16px">
                                Retirer
                            </button>
                        </form>
                    </div>
