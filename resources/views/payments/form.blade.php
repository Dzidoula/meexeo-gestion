<x-layouts.app title="Enregistrer un paiement — MEEXEO">
    ...
    <div class="mt-6 grid gap-5 lg:grid-cols-[1.4fr_1fr]" x-data="{
        ...
        <form method="POST" action="{{ route('payments.store') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            @if ($lease)
                ...
                <section class="rounded-meexeo border border-lin-clair bg-papier p-6">
                    ...
                    @error('month') <p class="mt-2 text-xs text-terre">{{ $message }}</p> @enderror
                </section>
                ...
                <section class="rounded-meexeo border border-lin-clair bg-papier p-6">
                    ...
                    <div class="mt-4 grid gap-4 sm:grid-cols-2">
                        <div>
                            <label for="amount" class="text-xs font-semibold text-ardoise">Montant encaissé (FCFA)</label>
                            ...
                            @error('amount') <p class="mt-1 text-xs text-terre">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label for="paid_on" class="text-xs font-semibold text-ardoise">Date de paiement</label>
                            <input id="paid_on" name="paid_on" type="date" required value="{{ old('paid_on', now()->toDateString()) }}"
                                   class="chiffre mt-1.5 min-h-[44px] w-full rounded-meexeo border border-lin px-3 text-sm">
                            @error('paid_on') <p class="mt-1 text-xs text-terre">{{ $message }}</p> @enderror
                        </div>
                    </div>
                </section>
                ...
                <section class="rounded-meexeo border border-lin-clair bg-papier p-6">
                    ...
                    <div class="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
                        @foreach ($methods as $value => $label)
                            <label class="flex min-h-[44px] cursor-pointer items-center justify-center rounded-meexeo border border-lin px-3 text-center text-sm has-[:checked]:border-cuivre">
                                <input type="radio" name="method" value="{{ $value }}" x-model="methode" class="sr-only">
                                {{ $label }}
                            </label>
                        @endforeach
                    </div>
                    @error('method') <p class="mt-2 text-xs text-terre">{{ $message }}</p> @enderror

                    <div class="mt-4" x-show="libellesReference[methode]" x-cloak>
                        <label for="reference" class="text-xs font-semibold text-ardoise" x-text="libellesReference[methode]"></label>
                        <input id="reference" name="reference" class="mt-1.5 min-h-[44px] w-full rounded-meexeo border border-lin px-3 text-sm">
                    </div>
                </section>

                <section class="rounded-meexeo border border-lin-clair bg-papier p-6">
                    <h2 class="font-titre text-lg">5. Preuve de paiement <span class="text-terre">— OBLIGATOIRE</span></h2>
                    <input type="file" name="proof" accept="image/*,.pdf" required class="mt-3 text-sm"
                           @change="preuveJointe = $event.target.files.length > 0">
                    @error('proof') <p class="mt-2 text-xs text-terre">{{ $message }}</p> @enderror
                </section>

                <button type="submit" :disabled="! preuveJointe"
                        class="min-h-[44px] w-full rounded-meexeo bg-cuivre px-5 text-sm font-semibold text-papier disabled:cursor-not-allowed disabled:opacity-50">
        ...
        @if ($lease)
            <aside class="h-fit rounded-meexeo border border-lin-clair bg-papier p-6">
                ...
                <dl class="mt-4 space-y-3 text-sm">
                    ...
                    <div class="flex justify-between border-t border-lin-pale pt-3 font-semibold">
                        <dt>Reste à payer</dt>
